          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr>
                ...
                <th style={{ ...thStyle, textAlign: "center" }}>FP Anchored</th>
                    ...
                    >
                      ...
                      <td style={{ ...tdStyle, textAlign: "center" }}>
                        <span style={{
                          ...
                          background: c.fpAnchored ? T.greenLight : T.redLight,
                          color: c.fpAnchored ? T.green : T.red,
                        }}>{c.fpAnchored ? "Yes" : "No"}</span>
                      </td>
...
// ─── Page: Trades ───
// Build fund-to-clients map from clientRecords holdings
const getFundClients = (fundCode) => {
  return clientRecords.filter(c =>
    c.holdings.some(h => h.funds.some(f => f.code === fundCode))
  ).map(c => {
    const matchingHoldings = c.holdings.filter(h => h.funds.some(f => f.code === fundCode));
    const totalInFund = matchingHoldings.reduce((sum, h) => {
      return sum + h.funds.filter(f => f.code === fundCode).reduce((s, f) => s + f.balance, 0);
    }, 0);
    const accounts = matchingHoldings.map(h => h.account);
    return { ...c, fundBalance: totalInFund, fundAccounts: accounts };
  });
};
...
const TradesPage = () => {
  ...
  // Fund detail view
  if (selectedFundCode) {
    const fundInfo = mfSalesList.find(f => f.code === selectedFundCode);
    const rbcFundInfo = rbcFunds.find(f => f.code === selectedFundCode);
    const fundClients = getFundClients(selectedFundCode);

    if (!fundInfo) { setSelectedFundCode(null); return null; }

    return (
      <div>
        <button onClick={() => setSelectedFundCode(null)}
          style={{ padding: "6px 14px", fontSize: 12, fontWeight: 600, border: `1px solid ${T.border}`, background: T.white, color: T.textMid, borderRadius: 3, cursor: "pointer", fontFamily: "'Source Sans 3', sans-serif", marginBottom: 16 }}>
          ← All Trades
        </button>

        {/* Fund header */}
        <div style={{ background: T.white, border: `1px solid ${T.borderLight}`, borderLeft: `4px solid ${T.navy}`, borderRadius: 4, padding: "22px 24px", marginBottom: 16 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              <div style={{ fontSize: 20, fontWeight: 700, color: T.navy, fontFamily: "'Source Sans 3', sans-serif", marginBottom: 4 }}>{fundInfo.fund}</div>
              <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                <span style={{ fontSize: 13, color: T.textMid, fontFamily: "'Source Code Pro', monospace" }}>{fundInfo.code}</span>
                <span style={{ background: T.blueLight, color: T.navy, fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 3 }}>Series {fundInfo.series}</span>
                {rbcFundInfo && <span style={{ fontSize: 11, color: T.textMid }}>MER {rbcFundInfo.mer.toFixed(2)}%</span>}
                {rbcFundInfo && (
                  <span style={{
                    fontSize: 11, fontWeight: 600, padding: "2px 8px", borderRadius: 3,
                    background: rbcFundInfo.risk === "Low" ? T.greenLight : rbcFundInfo.risk === "Low-Med" ? T.blueLight : rbcFundInfo.risk === "Medium" ? T.orangeLight : T.redLight,
                    color: rbcFundInfo.risk === "Low" ? T.green : rbcFundInfo.risk === "Low-Med" ? T.blue : rbcFundInfo.risk === "Medium" ? T.orange : T.red,
                  }}>{rbcFundInfo.risk} Risk</span>
                )}
              </div>
            </div>
          </div>
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 16 }}>
          <StatCard label="YTD Sales" value={fmtFull(fundInfo.ytdSales)} sub={`${fundInfo.clients} client transactions`} accent={T.navy} />
          <StatCard label="Avg Transaction" value={fmtFull(fundInfo.avgSize)} sub="Per client" accent={T.blue} />
          <StatCard label="Trailing Commission" value={`${fundInfo.trailing.toFixed(2)}%`} sub="Annual" accent={T.gold} />
          <StatCard label="Clients Holding" value={fundClients.length} sub={`${fundClients.filter(c => c.fpAnchored).length} FP Anchored`} accent={T.green} />
        </div>

        {/* Performance + Mix row if we have rbcFundInfo */}
        {rbcFundInfo && (
          <div style={{ display: "flex", gap: 12, marginBottom: 16 }}>
            <div style={{ flex: 1, background: T.white, border: `1px solid ${T.borderLight}`, borderRadius: 4, padding: "16px 20px" }}>
              <SectionTitle>Performance</SectionTitle>
              <div style={{ display: "flex", gap: 24, marginTop: 8 }}>
                {[{ label: "YTD", val: rbcFundInfo.ytd }, { label: "1 Yr", val: rbcFundInfo.y1 }, { label: "3 Yr", val: rbcFundInfo.y3 }, { label: "5 Yr", val: rbcFundInfo.y5 }].map(p => (
                  <div key={p.label} style={{ textAlign: "center" }}>
                    <div style={{ fontSize: 11, color: T.textLight, fontFamily: "'Source Sans 3', sans-serif", marginBottom: 4 }}>{p.label}</div>
                    <div style={{ fontSize: 18, fontWeight: 700, color: p.val >= 0 ? T.green : T.red, fontFamily: "'Source Sans 3', sans-serif" }}>
                      {p.val > 0 ? "+" : ""}{p.val}%
                    </div>
                  </div>
                ))}
            ...
            <div style={{ flex: 1, background: T.white, border: `1px solid ${T.borderLight}`, borderRadius: 4, padding: "16px 20px" }}>
              <SectionTitle>Asset Mix</SectionTitle>
              <div style={{ display: "flex", height: 24, borderRadius: 3, overflow: "hidden", marginTop: 8, marginBottom: 8 }}>
                <div style={{ width: `${rbcFundInfo.eq}%`, background: T.navy }} />
                <div style={{ width: `${rbcFundInfo.fi}%`, background: T.blue }} />
                <div style={{ width: `${rbcFundInfo.cash}%`, background: T.border }} />
              </div>
              <div style={{ display: "flex", gap: 20 }}>
                {[{ label: "Equity", pct: rbcFundInfo.eq, color: T.navy }, { label: "Fixed Inc", pct: rbcFundInfo.fi, color: T.blue }, { label: "Cash", pct: rbcFundInfo.cash, color: T.border }].map(a => (
                  <span key={a.label} style={{ fontSize: 12, color: T.textMid, fontFamily: "'Source Sans 3', sans-serif", display: "flex", alignItems: "center", gap: 6 }}>
        ...
        {/* Segment + FP Anchored donuts */}
        {(() => {
          const segData = [
            { name: "High Net Worth", value: fundClients.filter(c => c.segment === "High Net Worth").reduce((s, c) => s + c.fundBalance, 0), count: fundClients.filter(c => c.segment === "High Net Worth").length },
            { name: "Mass Affluent", value: fundClients.filter(c => c.segment === "Mass Affluent").reduce((s, c) => s + c.fundBalance, 0), count: fundClients.filter(c => c.segment === "Mass Affluent").length },
            { name: "Mass Market", value: fundClients.filter(c => c.segment === "Mass Market").reduce((s, c) => s + c.fundBalance, 0), count: fundClients.filter(c => c.segment === "Mass Market").length },
          ].filter(d => d.count > 0);
          const fpData = [
            { name: "FP Anchored", value: fundClients.filter(c => c.fpAnchored).reduce((s, c) => s + c.fundBalance, 0), count: fundClients.filter(c => c.fpAnchored).length },
            { name: "Not Anchored", value: fundClients.filter(c => !c.fpAnchored).reduce((s, c) => s + c.fundBalance, 0), count: fundClients.filter(c => !c.fpAnchored).length },
          ].filter(d => d.count > 0);
          const sizeBands = [
            { name: "<$25K", min: 0, max: 25000 },
            { name: "$25-50K", min: 25000, max: 50000 },
            { name: "$50-100K", min: 50000, max: 100000 },
            { name: "$100-250K", min: 100000, max: 250000 },
            { name: "$250K+", min: 250000, max: Infinity },
          ].map(b => ({
            name: b.name,
            value: fundClients.filter(c => c.fundBalance >= b.min && c.fundBalance < b.max).reduce((s, c) => s + c.fundBalance, 0),
            count: fundClients.filter(c => c.fundBalance >= b.min && c.fundBalance < b.max).length,
          })).filter(b => b.count > 0);
          return (
            <>
              ...
              {/* Transaction Size Distribution for this fund */}
              {sizeBands.length > 0 && (
                <div style={{ marginBottom: 16, background: T.white, border: `1px solid ${T.borderLight}`, borderRadius: 4, padding: "16px 20px" }}>
                  <SectionTitle>Balance Distribution</SectionTitle>
                  <ResponsiveContainer width="100%" height={160}>
                    <BarChart data={sizeBands} margin={{ left: 20, right: 20, top: 10, bottom: 0 }}>
                      ...
                      <Bar yAxisId="left" dataKey="value" name="Total Balance" fill={T.navy} radius={[3, 3, 0, 0]} barSize={40} />
                      <Bar yAxisId="right" dataKey="count" name="# Clients" fill={T.border} radius={[3, 3, 0, 0]} barSize={40} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              )}
        ...
        {/* Clients holding this fund */}
        <div style={{ background: T.white, border: `1px solid ${T.borderLight}`, borderRadius: 4 }}>
          <div style={{ padding: "14px 20px", borderBottom: `1px solid ${T.borderLight}` }}>
            <SectionTitle>Clients Holding This Fund ({fundClients.length})</SectionTitle>
          </div>
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr>
                  ...
                  <th style={thStyle}>Segment</th>
                  <th style={{ ...thStyle, textAlign: "right" }}>Balance in Fund</th>
                  <th style={thStyle}>Account(s)</th>
                  <th style={{ ...thStyle, textAlign: "right" }}>Total AUA</th>
                  <th style={{ ...thStyle, textAlign: "center" }}>FP Anchored</th>
                  <th style={thStyle}>PRS</th>
                </tr>
              </thead>
              <tbody>
                {fundClients.sort((a, b) => b.fundBalance - a.fundBalance).map(c => (
                  <tr key={c.id} style={{ transition: "background 0.15s" }} onMouseEnter={e => e.currentTarget.style.background = T.bg} onMouseLeave={e => e.currentTarget.style.background = T.white}>
                    <td style={{ ...tdStyle, fontWeight: 600, color: T.navy }}>{c.name}</td>
                    <td style={tdStyle}>
                      <span style={{
                        fontSize: 11, fontWeight: 600, padding: "2px 8px", borderRadius: 3,
                        background: c.segment === "High Net Worth" ? T.goldLight : c.segment === "Mass Affluent" ? T.blueLight : T.bg,
                    ...
                    <td style={{ ...tdStyle, textAlign: "right", fontWeight: 700, color: T.navy }}>{fmtFull(c.fundBalance)}</td>
                    <td style={tdStyle}>
                      <div style={{ display: "flex", gap: 3, flexWrap: "wrap" }}>
                        {c.fundAccounts.map(a => (
                          <span key={a} style={{ fontSize: 10, fontWeight: 600, padding: "2px 6px", borderRadius: 3, background: T.blueLight, color: T.navy, fontFamily: "'Source Sans 3', sans-serif" }}>{a}</span>
                        ))}
                      </div>
                    </td>
                    <td style={{ ...tdStyle, textAlign: "right" }}>{fmtFull(c.aua)}</td>
                    <td style={{ ...tdStyle, textAlign: "center" }}>
                      <span style={{
                        fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 3,
                        background: c.fpAnchored ? T.greenLight : T.redLight,
                ...
                {fundClients.length === 0 && (
                  <tr><td colSpan={7} style={{ ...tdStyle, textAlign: "center", color: T.textLight, padding: 30 }}>No clients hold this fund</td></tr>
                )}
              </tbody>
            </table>
          </div>
    ...
    <div>
      ...
      <div style={{ marginTop: 16, background: T.white, border: `1px solid ${T.borderLight}` }}>
        <div style={{ padding: "14px 20px", borderBottom: `1px solid ${T.borderLight}` }}>
          ...
          <div style={{ fontSize: 11, color: T.textLight, fontFamily: "'Source Sans 3', sans-serif", marginTop: 2 }}>Click a fund to see client breakdown</div>
        </div>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
            ...
            <tbody>
              {mfSalesList.map((f, i) => (
                ...
                >
                  <td style={{ ...tdStyle, fontWeight: 600, color: T.navy }}>
                    <span style={{ marginRight: 6, fontSize: 10, color: T.textLight }}>▶</span>
                    {f.fund}
                  </td>
...
const EVENT_CATEGORIES = {
  distribution: { label: "Distribution", color: T.green, bg: T.greenLight, icon: "💰" },
  fund_change: { label: "Fund Change", color: T.blue, bg: T.blueLight, icon: "🔄" },
  launch: { label: "New Launch", color: T.navy, bg: "#E0E8F0", icon: "🚀" },
};

const fundEvents = [
  {
    id: "fe1",
    title: "Year-End Capital Gains Distribution",
    category: "distribution",
    date: "2026-03-15",
    summary: "Significant capital gains distributions expected across 4 equity funds. Non-registered accounts will generate taxable events — proactive client conversations recommended before March tax slip season.",
    resources: [
      { title: "2025 Capital Gains Distribution Estimates", url: "https://www.rbcgam.com/en/ca/distribution-estimates" },
      { title: "Tax Planning Guide for IRPs", url: "https://advisor.rbcgam.com/tax-planning-2026" },
      { title: "Client Talking Points: Capital Gains", url: "https://advisor.rbcgam.com/resources/cap-gains-talking-points" },
    ],
    fundsImpacted: [
      { code: "RBF266", name: "RBC Canadian Dividend Fund", estDistribution: "$2.45/unit", taxImpact: "High", totalSales: 340000 },
      { code: "RBF617", name: "RBC Global Equity Focus Fund", estDistribution: "$3.12/unit", taxImpact: "High", totalSales: 245000 },
      { code: "RBF275", name: "RBC U.S. Equity Fund", estDistribution: "$1.88/unit", taxImpact: "Medium", totalSales: 100000 },
      { code: "RBF573", name: "RBC Select Growth Portfolio", estDistribution: "$0.95/unit", taxImpact: "Medium", totalSales: 310000 },
    ],
    clientsImpacted: [
      { id: 3, name: "Priya Sharma", accounts: ["Non-Reg"], funds: ["RBF617", "RBF275"], estTax: 4200, fpAnchored: true },
      { id: 7, name: "Linda Martinez", accounts: ["Non-Reg"], funds: ["RBF275"], estTax: 1800, fpAnchored: false },
      { id: 10, name: "James O'Brien", accounts: ["Non-Reg"], funds: ["RBF266"], estTax: 950, fpAnchored: true },
      { id: 11, name: "Catherine Tremblay", accounts: ["Non-Reg"], funds: ["RBF266"], estTax: 3600, fpAnchored: true },
      { id: 13, name: "Rachel Nguyen", accounts: ["Non-Reg"], funds: ["RBF290"], estTax: 420, fpAnchored: false },
    ],
  },
  {
    id: "fe2",
    title: "RBC Select Portfolio Series — MER Reduction",
    category: "fund_change",
    date: "2026-04-01",
    summary: "Management fee reductions effective April 1 across the Select Portfolio series. Series A MERs decreasing by 8-15bps. Positive client conversation opportunity — lower cost without any action needed.",
    resources: [
      { title: "MER Reduction Announcement", url: "https://www.rbcgam.com/en/ca/news/mer-reduction-select" },
      { title: "Updated Fund Facts — Select Series", url: "https://www.rbcgam.com/en/ca/fund-facts/select-portfolios" },
    ],
    fundsImpacted: [
      { code: "RBF572", name: "RBC Select Balanced Portfolio", estDistribution: "MER 1.98% → 1.85%", taxImpact: "N/A", totalSales: 620000 },
      { code: "RBF571", name: "RBC Select Conservative Portfolio", estDistribution: "MER 1.85% → 1.72%", taxImpact: "N/A", totalSales: 480000 },
      { code: "RBF573", name: "RBC Select Growth Portfolio", estDistribution: "MER 2.08% → 1.95%", taxImpact: "N/A", totalSales: 310000 },
      { code: "RBF570", name: "RBC Select Very Conservative", estDistribution: "MER 1.72% → 1.64%", taxImpact: "N/A", totalSales: 130000 },
    ],
    clientsImpacted: [
      { id: 1, name: "Margaret Chen", accounts: ["RRSP"], funds: ["RBF571"], estTax: null, fpAnchored: true },
      { id: 2, name: "David Thompson", accounts: ["RRSP", "TFSA"], funds: ["RBF572", "RBF573"], estTax: null, fpAnchored: true },
      { id: 4, name: "Jean-Pierre Dubois", accounts: ["TFSA"], funds: ["RBF572"], estTax: null, fpAnchored: false },
      { id: 6, name: "Michael Okafor", accounts: ["FHSA"], funds: ["RBF572"], estTax: null, fpAnchored: false },
      { id: 9, name: "Fatima Al-Hassan", accounts: ["RRSP"], funds: ["RBF572"], estTax: null, fpAnchored: false },
      { id: 12, name: "Ahmed Patel", accounts: ["TFSA"], funds: ["RBF572"], estTax: null, fpAnchored: false },
      { id: 14, name: "Brian Fowler", accounts: ["RRSP"], funds: ["RBF572"], estTax: null, fpAnchored: true },
      { id: 15, name: "Amira Hassan", accounts: ["FHSA"], funds: ["RBF572"], estTax: null, fpAnchored: false },
    ],
  },
  {
    id: "fe3",
    title: "Quarterly Distribution — Income Funds",
    category: "distribution",
    date: "2026-03-31",
    summary: "Quarterly income distributions for bond and income-oriented funds. Routine event — mostly impacts registered accounts (tax-sheltered). Flag any RRIF clients for withdrawal coordination.",
    resources: [
      { title: "Q1 2026 Distribution Schedule", url: "https://www.rbcgam.com/en/ca/distribution-schedule" },
    ],
    fundsImpacted: [
      { code: "RBF140", name: "RBC Bond Fund", estDistribution: "$0.18/unit", taxImpact: "Low", totalSales: 280000 },
      { code: "RBF220", name: "RBC Canadian Short-Term Income", estDistribution: "$0.12/unit", taxImpact: "Low", totalSales: 190000 },
      { code: "RBF290", name: "RBC Balanced Fund", estDistribution: "$0.22/unit", taxImpact: "Low", totalSales: 155000 },
    ],
    clientsImpacted: [
      { id: 5, name: "Sandra Williams", accounts: ["RRIF"], funds: ["RBF140"], estTax: null, fpAnchored: true },
      { id: 8, name: "Robert Kim", accounts: ["RRSP"], funds: ["RBF220"], estTax: null, fpAnchored: true },
      { id: 1, name: "Margaret Chen", accounts: ["RRSP"], funds: ["RBF140"], estTax: null, fpAnchored: true },
      { id: 10, name: "James O'Brien", accounts: ["RRSP"], funds: ["RBF140"], estTax: null, fpAnchored: true },
    ],
  },
  {
    id: "fe6",
    title: "RBC ESG Global Equity Fund Launch",
    category: "launch",
    date: "2026-05-01",
    summary: "New ESG-screened global equity fund launching May 1. Competitive MER at 1.95% (Series A). Targets clients interested in responsible investing without sacrificing returns. Early adoption incentive for first 90 days.",
    resources: [
      { title: "Fund Prospectus — RBC ESG Global Equity", url: "https://www.rbcgam.com/en/ca/fund-facts/esg-global-equity" },
      { title: "ESG Investing — Client FAQ", url: "https://advisor.rbcgam.com/resources/esg-faq" },
      { title: "Responsible Investing Seminar Kit", url: "https://advisor.rbcgam.com/resources/ri-seminar" },
    ],
    fundsImpacted: [
      { code: "NEW01", name: "RBC ESG Global Equity Fund", estDistribution: "New — launching May 1", taxImpact: "N/A", totalSales: 0 },
      { code: "RBF617", name: "RBC Global Equity Focus Fund", estDistribution: "Existing comparable", taxImpact: "N/A", totalSales: 245000 },
    ],
    clientsImpacted: [
      { id: 2, name: "David Thompson", accounts: ["RRSP", "TFSA"], funds: ["RBF573"], estTax: null, fpAnchored: true },
      { id: 13, name: "Rachel Nguyen", accounts: ["RRSP", "TFSA"], funds: ["RBF572"], estTax: null, fpAnchored: false },
      { id: 15, name: "Amira Hassan", accounts: ["TFSA", "FHSA"], funds: ["RBF573"], estTax: null, fpAnchored: false },
      { id: 6, name: "Michael Okafor", accounts: ["TFSA", "FHSA"], funds: ["RBF573"], estTax: null, fpAnchored: false },
    ],
  },
];
...
const FundEventsPage = () => {
  ...
  const selectedEvent = fundEvents.find(e => e.id === selectedEventId);

  // Landing: tile grid
  if (!selectedEvent) {
    const upcoming = [...fundEvents].sort((a, b) => new Date(a.date) - new Date(b.date));
    return (
      <div>
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 16 }}>
          <StatCard label="Active Events" value={fundEvents.length} sub="Across all categories" accent={T.navy} />
          <StatCard label="Clients Impacted" value={new Set(fundEvents.flatMap(e => e.clientsImpacted.map(c => c.id))).size} sub={`of ${clientRecords.length} total`} accent={T.orange} />
          <StatCard label="Funds Impacted" value={new Set(fundEvents.flatMap(e => e.fundsImpacted.map(f => f.code))).size} sub="Unique funds" accent={T.blue} />
          <StatCard label="Next Event" value={new Date(upcoming[0].date).toLocaleDateString("en-CA", { month: "short", day: "numeric" })} sub={upcoming[0].title.substring(0, 30) + "..."} accent={T.green} />
        </div>
          ...
          {upcoming.map(evt => {
            ...
            return (
              <div key={evt.id}
                ...
                style={{
                  background: T.white, border: `1px solid ${T.borderLight}`, borderLeft: `4px solid ${cat.color}`,
                  borderRadius: 4, padding: "18px 20px", cursor: "pointer", transition: "all 0.15s",
                }}
                onMouseEnter={e => { e.currentTarget.style.borderColor = cat.color; e.currentTarget.style.boxShadow = "0 2px 8px rgba(0,0,0,0.06)"; }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = T.borderLight; e.currentTarget.style.borderLeftColor = cat.color; e.currentTarget.style.boxShadow = "none"; }}
              >
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <span style={{
                      ...
                      background: cat.bg, color: cat.color, fontFamily: "'Source Sans 3', sans-serif", textTransform: "uppercase",
                    }}>{cat.label}</span>
                  </div>
                  <span style={{
                    fontSize: 11, fontWeight: 600, color: isPast ? T.textLight : T.text,
                    fontFamily: "'Source Sans 3', sans-serif",
                    textDecoration: isPast ? "line-through" : "none",
                  }}>{dateStr}</span>
                </div>
                <div style={{ fontSize: 14, fontWeight: 700, color: T.navy, fontFamily: "'Source Sans 3', sans-serif", marginBottom: 6, lineHeight: 1.3 }}>{evt.title}</div>
                <div style={{ fontSize: 12, color: T.textMid, fontFamily: "'Source Sans 3', sans-serif", lineHeight: 1.4, marginBottom: 12,
                  display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden",
                }}>{evt.summary}</div>
                <div style={{ display: "flex", gap: 16, fontSize: 11, color: T.textLight, fontFamily: "'Source Sans 3', sans-serif" }}>
                  <span><strong style={{ color: T.text }}>{evt.fundsImpacted.length}</strong> funds</span>
                  <span><strong style={{ color: T.text }}>{evt.clientsImpacted.length}</strong> clients impacted</span>
                  <span><strong style={{ color: T.text }}>{evt.resources.length}</strong> resources</span>
                </div>
              </div>
            );
          })}
  ...
  const dateStr = new Date(selectedEvent.date).toLocaleDateString("en-CA", { month: "long", day: "numeric", year: "numeric" });

  const allFundCodes = ["All", ...selectedEvent.fundsImpacted.map(f => f.code)];

  const filteredClients = selectedEvent.clientsImpacted.filter(c => {
    const matchesFund = fundFilter === "All" || c.funds.includes(fundFilter);
    const matchesSearch = !clientFilter || c.name.toLowerCase().includes(clientFilter.toLowerCase());
  ...
  return (
    <div>
      {/* Back button + header */}
      <button onClick={() => { setSelectedEventId(null); setFundFilter("All"); setClientFilter(""); }}
        style={{ padding: "6px 14px", fontSize: 12, fontWeight: 600, border: `1px solid ${T.border}`, background: T.white, color: T.textMid, borderRadius: 3, cursor: "pointer", fontFamily: "'Source Sans 3', sans-serif", marginBottom: 16 }}>
        ← All Events
      </button>

      {/* Event header card */}
      <div style={{ background: T.white, border: `1px solid ${T.borderLight}`, borderLeft: `4px solid ${cat.color}`, borderRadius: 4, padding: "22px 24px", marginBottom: 16 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
          <span style={{ fontSize: 22 }}>{cat.icon}</span>
          <span style={{ fontSize: 10, fontWeight: 700, padding: "3px 12px", borderRadius: 10, background: cat.bg, color: cat.color, fontFamily: "'Source Sans 3', sans-serif", textTransform: "uppercase" }}>{cat.label}</span>
          <span style={{ fontSize: 12, color: T.textMid, fontFamily: "'Source Sans 3', sans-serif" }}>{dateStr}</span>
        </div>
        <div style={{ fontSize: 20, fontWeight: 700, color: T.navy, fontFamily: "'Source Sans 3', sans-serif", marginBottom: 8 }}>{selectedEvent.title}</div>
        <div style={{ fontSize: 13, color: T.textMid, fontFamily: "'Source Sans 3', sans-serif", lineHeight: 1.6, maxWidth: 800 }}>{selectedEvent.summary}</div>
      </div>

      {/* Supporting Resources */}
      <div style={{ background: T.white, border: `1px solid ${T.borderLight}`, borderRadius: 4, padding: "16px 20px", marginBottom: 16 }}>
        <SectionTitle>Supporting Resources</SectionTitle>
          ...
          {selectedEvent.resources.map((r, i) => (
            <a key={i} href={r.url} target="_blank" rel="noopener noreferrer"
              style={{
                ...
                textDecoration: "none", transition: "all 0.15s", flex: "1 1 calc(33% - 8px)", minWidth: 220,
              }}
            ...
            >
              ...
              <div>
                <div style={{ fontSize: 12, fontWeight: 600, color: T.navy, fontFamily: "'Source Sans 3', sans-serif" }}>{r.title}</div>
                <div style={{ fontSize: 10, color: T.textLight, fontFamily: "'Source Sans 3', sans-serif", marginTop: 1 }}>{r.url.replace("https://", "").substring(0, 45)}...</div>
              </div>
              <span style={{ marginLeft: "auto", fontSize: 12, color: T.textLight }}>↗</span>
            </a>
          ))}
        </div>
      </div>
      ...
      {/* Funds Impacted */}
      <div style={{ background: T.white, border: `1px solid ${T.borderLight}`, borderRadius: 4, marginBottom: 16 }}>
        <div style={{ padding: "14px 20px", borderBottom: `1px solid ${T.borderLight}` }}>
          <SectionTitle>Funds Impacted</SectionTitle>
        </div>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr>
                ...
                <th style={thStyle}>Tax Impact</th>
                <th style={{ ...thStyle, textAlign: "right" }}>Your YTD Sales</th>
              </tr>
            </thead>
            <tbody>
              {selectedEvent.fundsImpacted.map((f, i) => (
                <tr key={i} style={{ transition: "background 0.15s" }} onMouseEnter={e => e.currentTarget.style.background = T.bg} onMouseLeave={e => e.currentTarget.style.background = T.white}>
                  <td style={{ ...tdStyle, fontWeight: 600, color: T.navy }}>{f.name}</td>
                  <td style={{ ...tdStyle, fontFamily: "'Source Code Pro', monospace", fontSize: 12, color: T.textMid }}>{f.code}</td>
                  <td style={{ ...tdStyle, fontSize: 12 }}>{f.estDistribution}</td>
                  <td style={tdStyle}>
                    {f.taxImpact !== "N/A" ? (
                      <span style={{
                        fontSize: 11, fontWeight: 600, padding: "2px 8px", borderRadius: 3,
                        background: f.taxImpact === "High" ? T.redLight : f.taxImpact === "Medium" ? T.orangeLight : T.greenLight,
                        color: f.taxImpact === "High" ? T.red : f.taxImpact === "Medium" ? T.orange : T.green,
                      }}>{f.taxImpact}</span>
                    ) : (
                      <span style={{ fontSize: 11, color: T.textLight }}>—</span>
                    )}
                  </td>
                  <td style={{ ...tdStyle, textAlign: "right", fontWeight: 600 }}>{f.totalSales > 0 ? fmtFull(f.totalSales) : "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {/* Clients Impacted */}
      <div style={{ background: T.white, border: `1px solid ${T.borderLight}`, borderRadius: 4 }}>
        <div style={{ padding: "14px 20px", borderBottom: `1px solid ${T.borderLight}`, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
          <SectionTitle>Clients Impacted ({filteredClients.length})</SectionTitle>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            {/* Fund filter */}
            <select value={fundFilter} onChange={e => setFundFilter(e.target.value)}
              style={{
                padding: "6px 12px", fontSize: 12, border: `1px solid ${T.border}`, borderRadius: 3,
                fontFamily: "'Source Sans 3', sans-serif", outline: "none", background: T.white, color: T.text,
              }}
            >
              {allFundCodes.map(c => (
                <option key={c} value={c}>{c === "All" ? "All Funds" : c}</option>
              ))}
            </select>
            {/* Client search */}
            <input
              ...
              style={{
                padding: "6px 12px", fontSize: 12, border: `1px solid ${T.border}`, borderRadius: 3,
                fontFamily: "'Source Sans 3', sans-serif", outline: "none", width: 160,
              }}
            />
          </div>
        </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr>
                ...
                <th style={thStyle}>Funds Affected</th>
                {selectedEvent.clientsImpacted.some(c => c.estTax) && <th style={{ ...thStyle, textAlign: "right" }}>Est. Tax Impact</th>}
                <th style={{ ...thStyle, textAlign: "center" }}>FP Anchored</th>
              </tr>
            </thead>
            <tbody>
              {filteredClients.map((c, i) => (
                <tr key={i} style={{ transition: "background 0.15s" }} onMouseEnter={e => e.currentTarget.style.background = T.bg} onMouseLeave={e => e.currentTarget.style.background = T.white}>
                  ...
                  <td style={tdStyle}>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 3 }}>
                      {c.funds.length > 0 ? c.funds.map(f => (
                        <span key={f} style={{ fontSize: 10, fontWeight: 600, padding: "2px 6px", borderRadius: 3, background: T.bg, color: T.textMid, fontFamily: "'Source Code Pro', monospace" }}>{f}</span>
                      )) : <span style={{ fontSize: 11, color: T.textLight }}>—</span>}
                    </div>
                  </td>
                  {selectedEvent.clientsImpacted.some(cl => cl.estTax) && (
                    <td style={{ ...tdStyle, textAlign: "right", fontWeight: 600, color: c.estTax ? T.red : T.textLight }}>
                      {c.estTax ? fmtFull(c.estTax) : "—"}
                    </td>
                  )}
                  <td style={{ ...tdStyle, textAlign: "center" }}>
                    <span style={{
              ...
              {filteredClients.length === 0 && (
                <tr><td colSpan={5} style={{ ...tdStyle, textAlign: "center", color: T.textLight, padding: 30 }}>No clients match the current filters</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
const WorkflowsPage = () => {
  ...
  const [pendingTemplate, setPendingTemplate] = useState(null); // template or "blank"
  ...
  const startCreation = (template) => {
    setPendingTemplate(template);
    setCreateType("prospect");
    setCreateClientId("");
    setCreateClientName("");
  };

  const confirmCreation = () => {
    if (createType === "client" && !createClientId.trim()) return;
    const isBlank = pendingTemplate === "blank";
    const newFlow = {
      id: "w" + Date.now(),
      name: isBlank ? "Untitled Workflow" : pendingTemplate.name,
      description: isBlank ? "" : pendingTemplate.description,
      clientType: createType,
      clientId: createType === "client" ? createClientId.trim() : null,
      clientName: createClientName.trim() || null,
      steps: isBlank
        ? [{ id: "s" + (++stepCounter), title: "Step 1", desc: "", tools: [], docs: [], status: "not_started" }]
        : pendingTemplate.steps.map(s => ({ ...s, id: "s" + (++stepCounter), status: "not_started" })),
    };
  ...
  if (pendingTemplate) {
    ...
    return (
      <div>
        <div style={{ maxWidth: 480, margin: "40px auto", background: T.white, border: `1px solid ${T.borderLight}`, borderRadius: 6, padding: "28px 32px" }}>
          <div style={{ fontSize: 16, fontWeight: 700, color: T.navy, fontFamily: "'Source Sans 3', sans-serif", marginBottom: 4 }}>
            {isBlank ? "New Blank Workflow" : `Clone: ${pendingTemplate.name}`}
          </div>
          <div style={{ fontSize: 12, color: T.textLight, fontFamily: "'Source Sans 3', sans-serif", marginBottom: 20 }}>
            Link this workflow to a prospect or existing client.
          </div>

          {/* Type selector */}
          <div style={{ fontSize: 11, fontWeight: 700, color: T.textMid, fontFamily: "'Source Sans 3', sans-serif", marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.3px" }}>Type</div>
          <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
            {[{ id: "prospect", label: "👤 Prospect", desc: "New lead, not yet in system" }, { id: "client", label: "📋 Client", desc: "Existing client with ID" }].map(opt => (
              <div key={opt.id} onClick={() => setCreateType(opt.id)} style={{
                flex: 1, padding: "14px 16px", borderRadius: 4, cursor: "pointer", transition: "all 0.15s",
                border: `2px solid ${createType === opt.id ? T.blue : T.borderLight}`,
                background: createType === opt.id ? T.blueLight : T.white,
              }}>
                <div style={{ fontSize: 14, fontWeight: 700, color: createType === opt.id ? T.navy : T.text, fontFamily: "'Source Sans 3', sans-serif" }}>{opt.label}</div>
                <div style={{ fontSize: 11, color: T.textMid, fontFamily: "'Source Sans 3', sans-serif", marginTop: 2 }}>{opt.desc}</div>
              </div>
            ))}
          </div>
          ...
          {createType === "client" && (
            <div style={{ marginBottom: 20 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: T.textMid, fontFamily: "'Source Sans 3', sans-serif", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.3px" }}>Client ID <span style={{ color: T.red }}>*</span></div>
              <input
                value={createClientId}
                onChange={e => setCreateClientId(e.target.value)}
                placeholder="e.g. CL-20489"
                style={{
                  width: "100%", padding: "10px 14px", fontSize: 14, border: `1px solid ${!createClientId.trim() && createClientId !== "" ? T.red : T.border}`,
                  borderRadius: 3, fontFamily: "'Source Code Pro', monospace", outline: "none", boxSizing: "border-box", marginBottom: 10,
                }}
              />
              <div style={{ fontSize: 11, fontWeight: 700, color: T.textMid, fontFamily: "'Source Sans 3', sans-serif", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.3px" }}>Client Name</div>
              <input
                value={createClientName}
                onChange={e => setCreateClientName(e.target.value)}
                placeholder="e.g. Margaret Chen"
                style={{
                  width: "100%", padding: "10px 14px", fontSize: 14, border: `1px solid ${T.border}`,
          ...
          {createType === "prospect" && (
            <div style={{ marginBottom: 20 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: T.textMid, fontFamily: "'Source Sans 3', sans-serif", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.3px" }}>Prospect Name</div>
              <input
                value={createClientName}
                onChange={e => setCreateClientName(e.target.value)}
          ...
          <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
            <button onClick={cancelCreation} style={{
              padding: "9px 20px", fontSize: 13, fontWeight: 600, border: `1px solid ${T.border}`,
              background: T.white, color: T.textMid, borderRadius: 3, cursor: "pointer", fontFamily: "'Source Sans 3', sans-serif",
            }}>Cancel</button>
            <button onClick={confirmCreation} disabled={createType === "client" && !createClientId.trim()} style={{
              padding: "9px 20px", fontSize: 13, fontWeight: 600, border: "none",
              background: (createType === "client" && !createClientId.trim()) ? T.border : T.navy,
              color: (createType === "client" && !createClientId.trim()) ? T.textLight : T.white,
              borderRadius: 3, cursor: (createType === "client" && !createClientId.trim()) ? "not-allowed" : "pointer", fontFamily: "'Source Sans 3', sans-serif",
  ...
  if (showTemplates || workflows.length === 0) {
    return (
      <div>
        ...
        <button onClick={() => startCreation("blank")} style={{
          ...
          return (
            <div key={w.id} onClick={() => setActiveId(w.id)} style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 5 }}>
                <span style={{
                  fontSize: 9, fontWeight: 700, padding: "1px 6px", borderRadius: 2, textTransform: "uppercase", fontFamily: "'Source Sans 3', sans-serif",
                  background: w.clientType === "client" ? T.blueLight : T.goldLight,
                  color: w.clientType === "client" ? T.navy : T.gold,
                }}>{w.clientType}</span>
                <span style={{ fontSize: 10, color: T.textMid, fontFamily: "'Source Sans 3', sans-serif", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  {w.clientName || w.clientId || "—"}
                </span>
              </div>
      ...
      {activeFlow ? (
        <div style={{ flex: 1, overflowY: "auto" }}>
          ...
          <div style={{ background: T.white, border: `1px solid ${T.borderLight}`, borderRadius: 4, padding: "16px 20px", marginBottom: 16 }}>
            ...
            />
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 10, paddingTop: 10, borderTop: `1px solid ${T.borderLight}` }}>
              <span style={{
                fontSize: 10, fontWeight: 700, padding: "2px 10px", borderRadius: 3, textTransform: "uppercase", fontFamily: "'Source Sans 3', sans-serif",
                background: activeFlow.clientType === "client" ? T.blueLight : T.goldLight,
                color: activeFlow.clientType === "client" ? T.navy : T.gold,
              }}>{activeFlow.clientType === "client" ? "📋 Client" : "👤 Prospect"}</span>
              {activeFlow.clientName && (
                <span style={{ fontSize: 13, fontWeight: 600, color: T.text, fontFamily: "'Source Sans 3', sans-serif" }}>{activeFlow.clientName}</span>
              )}
              {activeFlow.clientId && (
                <span style={{ fontSize: 12, color: T.textLight, fontFamily: "'Source Code Pro', monospace" }}>{activeFlow.clientId}</span>
              )}
            </div>
          </div>
